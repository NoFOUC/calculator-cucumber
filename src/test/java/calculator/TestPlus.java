...
class TestPlus {
	...
	@BeforeEach
	void setUp() {
		  params = new ArrayList<>(Arrays.asList(new IntegerValue(value1),new IntegerValue(value2)));
	...
	void testEquals() {
		...
		ArrayList<Expression> p = new ArrayList<>(Arrays.asList(new IntegerValue(value1), new IntegerValue(value2)));
		try {
			Plus e = new Plus(p, Notation.INFIX);
			assertEquals(op, e);
			assertEquals(e, e);
			assertNotEquals(e, new Plus(new ArrayList<>(Arrays.asList(new IntegerValue(5), new IntegerValue(4))), Notation.INFIX));
		}
		catch(IllegalConstruction e) { fail(); }
	}
